...
一个面向 Windows x64、以稳定输入和明确安全边界为核心的独立 KVM 客户端，正在为开源发布完善。
...
> 上图由本项目的本地环回冒烟测试生成。画面不包含真实设备、账户、凭据或管理地址。
...
所有截图均由连接 `127.0.0.1` 的桌面冒烟测试生成，展示的是合成状态与无视频画面，不包含生产环境信息。
...
自包含发布会携带所需 .NET 运行时，目标 Windows 机器无需单独安装 SDK 或运行时。项目当前未提供代码签名和安装包；从非受信来源获取二进制文件时，请先核对发布者提供的哈希。
...
欢迎提交经过验证的协议兼容、稳定性、可访问性、本地化和测试改进。为了让远程管理工具保持可审计，贡献应遵循以下约定：
...
对安全问题，请不要在公开 Issue 中附带可利用细节、真实目标或任何凭据。在仓库建立正式安全报告渠道之前，请先提交不含敏感细节的维护者联系请求。

## 许可证

本仓库当前尚未包含开源许可证文件。在选择并添加明确许可证之前，源代码仍受默认著作权规则约束，不能仅因仓库可见就推定为允许复制、修改或再发布。正式公开发布和接受外部贡献前必须先完成许可证选择。
...
<p align="center"><sub>截至 2026-07-16 · 20 commits · 数据来源：当前仓库 Git 历史</sub></p>
