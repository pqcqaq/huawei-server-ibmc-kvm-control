...
<p><strong>CROSS-PLATFORM REMOTE CONSOLE</strong></p>

<h1>iBMC KVM</h1>

**面向 Windows 与 Linux 的 iBMC / Linux 远程控制台**

<p>无需 Java 运行时 · 无需浏览器插件 · 不安装系统级键鼠钩子</p>

一个面向 Windows/Linux x64、以稳定输入和明确安全边界为核心的独立 KVM 客户端与 Rust Linux Agent。
...
<p>
  ...
  <img src="https://img.shields.io/badge/UI-WPF%20%2B%20Avalonia-0078D4?style=flat-square" alt="WPF and Avalonia">
  <img src="https://img.shields.io/badge/platform-Windows%20%2F%20Linux%20x64-0078D4?style=flat-square" alt="Windows and Linux x64">
  ...
  <img src="https://img.shields.io/badge/tests-.NET%20%2B%20Rust-16A34A?style=flat-square" alt=".NET and Rust tests">
</p>
...
iBMC KVM 使用 .NET 9 提供 Windows WPF 客户端和 Windows/Linux Avalonia 客户端，并使用 Rust 提供 X11 Linux Agent。它把服务器视频、USB HID 键盘与鼠标、虚拟媒体、录像、断线恢复和机箱多会话管理整合到原生 x64 应用中，同时把输入影响范围严格限制在远端画面窗口内。
...
- WPF 使用 Windows DPAPI 保存连接；Linux Avalonia 使用 GNOME Secret Service 保管 AES-GCM 密钥。
...
- 原生 Windows/Linux x64 桌面应用，可发布为不依赖本机 .NET 安装的自包含程序；
...
| Linux Agent | X11 桌面捕获、绝对鼠标、HID 键盘、TLS 配对与自动重连 | 已实现，不包含电源、Shell、文件传输或虚拟媒体 |
...
项目把桌面呈现、会话编排与线协议分层。WPF 和 Avalonia 都不解析 iBMC 网络包；Linux Agent 会话在 Core 中使用独立、受边界限制的 TLS 协议，不复用或模拟硬件 iBMC 协议。

```mermaid
flowchart LR
    subgraph Desktop[桌面呈现]
        WPF[IbmcKvm.App · Windows WPF]
        Avalonia[IbmcKvm.Desktop · Windows / Linux Avalonia]
    end
    ...
    subgraph Core[IbmcKvm.Core · 会话与工作流]
        ...
        Media[虚拟媒体与 SCSI 处理]
        Chassis[机箱多会话协调]
        AgentSession[Agent TLS 会话与 JPEG tile 解码]
    end
    ...
    Device[(iBMC / iMana)]
    LinuxAgent[(Rust Linux Agent · X11)]
    ...
    AgentSession -->|Agent Protocol v1 / TLS| LinuxAgent
```

一次连接的主要数据流如下：

1. **发现与认证**：规范化目标地址并探测证书；iBMC 根据设备响应选择登录配置，Agent 使用证书指纹和高熵配对令牌。
2. **能力协商**：iBMC 路径解析权限、加密、输入、虚拟媒体和机箱能力；Agent 路径协商分辨率、帧率和键鼠能力。
3. **视频接收**：iBMC 路径组装硬件块与差分画面；Agent 路径合成 JPEG tile；桌面层只呈现最终位图。
4. **输入发送**：聚焦事件进入 HID 状态机，状态机按顺序产生键盘或鼠标报告，再由当前会话编码发送。
5. **故障恢复**：会话监督器按有界策略重建 iBMC 或 Agent 连接；iBMC 视频恢复后再恢复已挂载介质。
...
- [ADR-0004：跨平台 Avalonia 桌面客户端](docs/adr/0004-cross-platform-avalonia-desktop.md)
- [ADR-0005：Rust Linux Agent](docs/adr/0005-rust-linux-agent.md)
...
- “记住此连接”默认关闭；
- WPF 连接配置写入 `%LOCALAPPDATA%\IbmcKvm\connection-settings.bin` 并由 Windows DPAPI 加密；
- Linux Avalonia 使用 AES-GCM 加密连接文件，密钥保存在当前用户的 GNOME Secret Service 中，文件权限限制为当前用户；
- 会话密钥不写入日志，也不作为设置持久化；
- 自签名证书在首次使用前展示主题、颁发者、有效期和 SHA-256 指纹；
- 证书信任按主机与端口限定，可检查、导入或撤销，不修改操作系统全局证书信任。
...
- Windows 10/11 x64，或使用 X11 桌面的 Linux x64；
- 从源码构建需要 .NET 9 SDK；
- 构建 Linux Agent 需要 Rust 1.90 或更高版本；
...
dotnet run --project src/IbmcKvm.Desktop/IbmcKvm.Desktop.csproj --configuration Release
```

启动后可选择 Huawei iBMC 或 Linux Agent。连接 iBMC 时输入管理地址和账户凭据；连接 Agent 时输入 `HOST:7443` 和配对令牌。遇到自签名证书时，请先通过其他可信渠道核对 SHA-256 指纹。

### Linux Agent

Rust Agent 用于远程控制已经登录的 X11 桌面，不模拟 iBMC，也不提供电源、命令执行、文件传输或虚拟媒体接口。
...
保存 `init` 输出的证书指纹与配对令牌，然后在客户端选择“Linux Agent”。完整的 systemd 用户服务配置见 [`agent/README.md`](agent/README.md)。

自动化冒烟测试可以安全地从文件读取令牌，避免令牌出现在进程参数中：
...
3. 如需保存连接，启用“加密记住此连接”；凭据只保存在当前操作系统用户的加密本地配置中。
...
生成 Linux Avalonia 客户端和 Rust Agent：

```bash
dotnet publish src/IbmcKvm.Desktop/IbmcKvm.Desktop.csproj \
...
解决方案包含 400 多项 .NET 与 Rust 自动化测试，覆盖协议、密码学、视频、输入、会话、录像、虚拟媒体、设置和 UI 规则。桌面冒烟测试只连接本机环回服务，并明确拒绝电源或 USB 重置命令。
...
- `.rep`、AVI、证书信任、DPAPI / Secret Service 设置及多语言 UI。
...
src/
  ...
  IbmcKvm.Desktop/       Windows/Linux Avalonia 桌面客户端
agent/                   Rust X11 Agent、TLS 协议服务和 systemd 单元
tests/
  IbmcKvm.Protocol.Tests/
  IbmcKvm.Core.Tests/
  IbmcKvm.App.Tests/
  IbmcKvm.Desktop.Tests/
  IbmcKvm.LinuxSmoke/
...
Avalonia 客户端支持 Windows/Linux x64。Rust Agent 第一版控制已登录的 X11 桌面；Wayland 无人值守、登录前控制、固件画面和关机后的硬件 KVM 不在此能力范围内。
...
本项目使用宽松的 [MIT License](LICENSE)。在保留版权声明和许可声明的前提下，可以使用、复制、修改、合并、发布、分发、再许可和销售本软件；软件按“原样”提供，不附带任何明示或默示担保。
...
<p align="center"><sub>截至 2026-07-17 · 24 commits · 数据来源：当前仓库 Git 历史</sub></p>
